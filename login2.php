...
<head>
    ...
    <style type="text/css">
        ...
        .bg::before {
            content: '';
            background-image: url('./asset/img/foto_kua.png');
                ...
                <div class="card-panel z-depth-2" id="login">
                    ...
                    <div class="col s12">
                        <div class="card-content">
                            <h5 class="center" id="title">Aplikasi Sistem Informasi</h5>
                    ...
                    <?php
                        if(isset($_REQUEST['submit'])){
                            ...
                            if($_REQUEST['username_catin'] == "" || $_REQUEST['pass_catin'] == ""){
                                ...
                                $username_catin = trim(htmlspecialchars(mysqli_real_escape_string($config, $_REQUEST['username_catin'])));
                                $pass_catin = trim(htmlspecialchars(mysqli_real_escape_string($config, $_REQUEST['pass_catin'])));

                                $query = mysqli_query($config, "SELECT id_catin, username_catin, nama_suami FROM tbl_catin WHERE username_catin=BINARY'$username_catin' AND pass_catin=MD5('$pass_catin')");

                                if(mysqli_num_rows($query) > 0){
                                    list($id_catin, $username_catin, $nama_suami, $nip, $admin) = mysqli_fetch_array($query);

                                    //buat session
                                    $_SESSION['id_catin'] = $id_catin;
                                    $_SESSION['username_catin'] = $username_catin;
                                    $_SESSION['nama_suami'] = $nama_suami;
                    ...
                    <form class="col s12 m12 offset-4 offset-4" method="POST" action="" >
                        ...
                        <div class="input-field col s12">
                            ...
                            <input id="username_catin" type="text" class="validate" name="username_catin" required>
                            <label for="username_catin">Username</label>
                        </div>
                        <div class="input-field col s12">
                            <i class="material-icons prefix md-prefix">lock</i>
                            <input id="pass_catin" type="password" class="validate" name="pass_catin" required>
                            <label for="pass_catin">Password</label>
                        </div>
                        <div class="input-field col s12">
                            <button type="submit" class="btn-large waves-effect waves-light blue-grey col s12" name="submit">LOGIN</button>
                        </div>
                        <div class="input-field col s12">
                            <a href="index.php" class="btn-large waves-effect waves-light blue-grey col s12" name="submit">Kembali</a>
                        </div>
                    </form>
                    <!-- Form END -->
                    <?php
